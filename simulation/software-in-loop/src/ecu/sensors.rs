...
pub fn initialize_sensors(noise_config: &PyDict) -> HashMap<EcuSensor, Box<dyn SensorNoise>> {
    ...
    for sensor_variant in EcuSensor::iter() {
        match sensor_variant {
            ...
            EcuSensor::IgniterThroatTemperature | EcuSensor::EngineThroatTemperature => {
...
pub fn setup_pressure_sensor_noise(
    ...
    if let Some(sensor_noise_config) = noise_config.get_item(format!("{:?}", sensor)) {
        let rate = sensor_noise_config
            .get_item("rate")
            .expect(&format!(
                "Failed to get rate from noise_config for {:?}",
                sensor
            ))
            .extract::<f64>()
            .expect(&format!(
                "Failed to extract rate from noise_config for {:?}",
                sensor
            ));
        ...
        let std_dev = sensor_noise_config
            .get_item("std_dev")
            .expect(&format!(
                "Failed to get std_dev from noise_config for {:?}",
                sensor
            ))
            .extract::<f64>()
            .expect(&format!(
                "Failed to extract std_dev from noise_config for {:?}",
                sensor
            ));
        ...
        let pressure_min = sensor_noise_config
            ...
            .expect(&format!(
                "Failed to get pressure_min from noise_config for {:?}",
                sensor
            ))
            .extract::<f64>()
            .expect(&format!(
                "Failed to extract pressure_min from noise_config for {:?}",
                sensor
            ));
        ...
        let pressure_max = sensor_noise_config
            ...
            .expect(&format!(
                "Failed to get pressure_max from noise_config for {:?}",
                sensor
            ))
            .extract::<f64>()
            .expect(&format!(
                "Failed to extract pressure_max from noise_config for {:?}",
                sensor
            ));

        sensors.insert(
            sensor,
            Box::new(LinearVoltagePressureTranducer::new(
                rate,
                pressure_min,
                pressure_max,
                410,
...
pub fn setup_temperature_sensor_noise(
    ...
    sensors.insert(
        sensor,
        Box::new(LinearVoltageTemperatureSensor::new(
            0.1, 0.0, 1.0, 0, 4095, 0.1,
        )),
    );
}
